...
assert.equal(usageDashboard.users.find((user) => user.userId === requester.id)?.opensAllTime, 1);
assert.equal(usageDashboard.totals.opensAllTime, 1);
assert.throws(() => m.getUsageDashboard(admin.id), /Developer access is required/i);
