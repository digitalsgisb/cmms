...
function RequesterElapsedTimer({ workOrder, timerNow, detailed = false }: { workOrder: WorkOrder; timerNow: string; detailed?: boolean }) {
  ...
  const label = running ? "Live time since opened" : workOrder.status === "closed" ? "Total time: open to close" : "Time open before cancellation";
  return <div className={`requester-elapsed-timer ${running ? "is-live" : "is-stopped"} ${detailed ? "is-detailed" : ""}`}><Clock3 size={detailed ? 21 : 17} aria-hidden="true" /><span><small>{label}</small><strong>{formatLiveDuration(workOrder.createdAt, end)}</strong></span>{detailed ? <time>{running ? "Counting live" : `Closed ${formatDateTime(workOrder.closedAt || workOrder.updatedAt)}`}</time> : null}</div>;
}
