...
export function PublicRequesterPage() {
  ...
  async function loadAccountWorkOrders() {
    ...
    setWorkOrders(next.filter((workOrder) => workOrder.requesterId === currentUser.id));
  }
  ...
  useEffect(() => {
    if (signedRequester) {
      ...
      setWorkOrders([]); setView("new"); setSelectedType(null);
    }
  }, [currentUser?.id]);
  ...
  const stats = useMemo(() => ({
    open: workOrders.filter((item) => statusesByFilter.open.includes(item.status)).length,
    in_progress: workOrders.filter((item) => statusesByFilter.in_progress.includes(item.status)).length,
    waiting: workOrders.filter((item) => statusesByFilter.waiting.includes(item.status)).length,
    closed: workOrders.filter((item) => statusesByFilter.closed.includes(item.status)).length
  }), [workOrders]);
  const pendingVerification = useMemo(() => workOrders.filter((item) => item.status === "resolved"), [workOrders]);
  const visibleWorkOrders = useMemo(() => {
    const byStatus = statusFilter === "all" ? workOrders : workOrders.filter((item) => statusesByFilter[statusFilter].includes(item.status));
  ...
  function openView(next: RequesterView) {
    ...
    if (next !== "new") setSelectedType(null);
  ...
  async function submitLogin(event: FormEvent) {
    ...
    try {
      ...
      if (user.role !== "requester") {
        logout(); setLoginError("This sign-in is for production requester accounts. Staff can use the main CMMS sign-in."); return;
      }
  ...
  return <div className={`requester-app-shell ${signedRequester ? "is-account" : "is-guest"}`}>
    <header className="requester-app-topbar">
      <div className="requester-app-brand"><span><img src="/brand/sugi_symbol.png" alt="Sugihara Grand" /></span><div><small>SUGI CMMS</small><strong>{signedRequester ? "Production Requester" : "Guest Request"}</strong></div></div>
      <div className="requester-app-account-action">{signedRequester ? <button type="button" onClick={() => openView("account")}><UserCircle2 size={18} /><span>{currentUser.name}</span></button> : currentUser ? <a href="/"><Home size={17} />Return to CMMS</a> : <button type="button" onClick={() => setLoginOpen(true)}><LogIn size={17} />Production sign in</button>}</div>
    </header>
    ...
    <main className="requester-app-main">
      ...
      {signedRequester && view === "dashboard" ? <RequesterDashboard user={currentUser} workOrders={workOrders} stats={stats} pendingVerification={pendingVerification} onStatus={showStatus} onView={openView} onDetail={openDetail} /> : null}
      {view === "new" ? <section className={`requester-new-view ${selectedType ? "" : "requester-new-view-locked"}`} aria-hidden={!selectedType}>
        ...
        {selectedType ? <RequesterForm selectedType={selectedType} form={form} setForm={setForm} isOffice={isOffice} sectionOptions={sectionOptions} machineOptions={machineOptions} issueCategoryOptions={issueOptions} issueFiles={issueFiles} setIssueFiles={setIssueFiles} submitting={submitting} signedRequester={signedRequester} onChangeType={() => setSelectedType(null)} onSubmit={submit} /> : <section className="requester-form-panel requester-form-locked"><div className="requester-panel-heading"><span className="requester-panel-icon"><ShieldCheck size={18} /></span><div><h2>Choose a category</h2><span>The request form opens after your selection.</span></div></div></section>}
      </section> : null}
      ...
      {signedRequester && view === "account" ? <section className="requester-account-view"><div className="requester-account-avatar">{initialsFor(currentUser.name)}</div><p>Production requester</p><h1>{currentUser.name}</h1><span>{currentUser.title}</span><dl><div><dt>Department</dt><dd>{currentUser.department}</dd></div><div><dt>Username</dt><dd>{currentUser.username}</dd></div><div><dt>Tracked requests</dt><dd>{workOrders.length}</dd></div></dl><PwaInstallButton /><button className="requester-signout" type="button" onClick={() => { logout(); setSuccess("Signed out. You can continue as a guest."); }}><LogOut size={17} />Sign out and continue as guest</button></section> : null}
    </main>
...
function RequesterForm({ selectedType, form, setForm, isOffice, sectionOptions, machineOptions, issueCategoryOptions, issueFiles, setIssueFiles, submitting, signedRequester, onChangeType, onSubmit }: {
  selectedType: WorkOrderType; form: typeof initialRequesterForm; setForm: React.Dispatch<React.SetStateAction<typeof initialRequesterForm>>; isOffice: boolean;
