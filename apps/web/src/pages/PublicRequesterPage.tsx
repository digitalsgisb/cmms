...
return <section className="requester-verify-view"><div className="requester-view-heading"><div><p>Requester decision</p><h1>Verify Completed Work</h1><span>Check the repair result before closing or returning the job.</span></div><ShieldCheck size={28} /></div>{workOrders.length ? <div className="requester-verification-list">{workOrders.map((workOrder) => <article className="requester-verification-card" key={workOrder.id} onClick={(event) => { if (!(event.target as HTMLElement).closest("button, textarea, input, label")) onDetail(workOrder); }}><div className="card-topline"><strong>{workOrder.number}</strong><StatusBadge status={workOrder.status} /></div><h2>{workOrder.machineName || workOrder.location}</h2><p>{workOrder.issueDescription}</p>{workOrder.completionNote ? <blockquote><strong>Maintenance note</strong>{workOrder.completionNote}</blockquote> : null}<button className="requester-view-evidence" type="button" disabled={detailLoading} onClick={() => onDetail(workOrder)}><Eye size={16} />View timeline and completion photos</button><label>Verification note <small>Required only when returning</small><textarea rows={3} value={notes[workOrder.id] || ""} onChange={(event) => onNote(workOrder.id, event.target.value)} placeholder="Add your verification note" /></label><div className="requester-verification-actions"><button type="button" className="verify" disabled={Boolean(actionId)} onClick={() => onVerify(workOrder, "closed")}><CheckCircle2 size={17} />{actionId === workOrder.id ? "Saving..." : "Verify & Close"}</button><button type="button" className="return" disabled={Boolean(actionId)} onClick={() => onVerify(workOrder, "returned")}><RefreshCcw size={17} />Return to Maintenance</button></div></article>)}</div> : <RequesterEmpty title="Nothing to verify" copy="When maintenance resolves one of your work orders, it will appear here." />}</section>;
}
