...
function needsProductionDowntimeExplanation(workOrder: WorkOrder) {
  const resolvedAt = workOrder.resolvedAt || (workOrder.status === "resolved" ? workOrder.updatedAt : null);
  return workOrder.responsibleDepartment === "Production" && Boolean(resolvedAt) &&
    Math.max(0, Date.parse(resolvedAt!) - Date.parse(workOrder.createdAt)) >= longProductionDowntimeMinutes * 60000;
}
...
export function PublicRequesterPage() {
  ...
  async function verifyWorkOrder(workOrder: WorkOrder, status: "closed" | "returned") {
    ...
    if (status === "closed" && needsProductionDowntimeExplanation(workOrder) && !note) { setError(`Add a production downtime explanation before closing work orders lasting ${longProductionDowntimeMinutes} minutes or more.`); return; }
  ...
  return <div className={`requester-app-shell ${signedRequester ? "is-account" : "is-guest"}`}>
    ...
    {detail ? <RequesterDetailDialog detail={detail} canVerify={detail.requesterId === currentUser?.id} timerNow={timerNow} onClose={() => setDetail(null)} onVerify={verifyWorkOrder} actionId={actionId} note={verificationNotes[detail.id] || ""} onNote={(note) => setVerificationNotes((current) => ({ ...current, [detail.id]: note }))} /> : null}
  </div>;
}
...
return <section className="requester-verify-view"><div className="requester-view-heading"><div><p>Requester decision</p><h1>Verify Completed Work</h1><span>Check the repair result before closing or returning the job.</span></div><ShieldCheck size={28} /></div>{workOrders.length ? <div className="requester-verification-list">{workOrders.map((workOrder) => { const needsReason = needsProductionDowntimeExplanation(workOrder); return <article className="requester-verification-card" key={workOrder.id} onClick={(event) => { if (!(event.target as HTMLElement).closest("button, textarea, input, label")) onDetail(workOrder); }}><div className="card-topline"><strong>{workOrder.number}</strong><StatusBadge status={workOrder.status} /></div><RequesterElapsedTimer workOrder={workOrder} timerNow={timerNow} /><h2>{workOrder.machineName || workOrder.location}</h2><p>{workOrder.issueDescription}</p>{workOrder.completionNote ? <blockquote><strong>Maintenance note</strong>{workOrder.completionNote}</blockquote> : null}<button className="requester-view-evidence" type="button" disabled={detailLoading} onClick={() => onDetail(workOrder)}><Eye size={16} />View timeline and completion photos</button><label>{needsReason ? "Production downtime explanation" : "Verification note"} <small>{needsReason ? `Required: downtime reached ${longProductionDowntimeMinutes}+ minutes` : "Required only when returning"}</small><textarea rows={3} value={notes[workOrder.id] || ""} onChange={(event) => onNote(workOrder.id, event.target.value)} placeholder={needsReason ? "Explain the operational reason for the extended downtime" : "Add your verification note"} /></label><div className="requester-verification-actions"><button type="button" className="verify" disabled={Boolean(actionId) || (needsReason && !notes[workOrder.id]?.trim())} onClick={() => onVerify(workOrder, "closed")}><CheckCircle2 size={17} />{actionId === workOrder.id ? "Saving..." : "Verify & Close"}</button><button type="button" className="return" disabled={Boolean(actionId)} onClick={() => onVerify(workOrder, "returned")}><RefreshCcw size={17} />Return to Maintenance</button></div></article>; })}</div> : <RequesterEmpty title="Nothing to verify" copy="When maintenance resolves one of your work orders, it will appear here." />}</section>;
}
...
function RequesterWorkOrderCard({ workOrder, timerNow, onDetail, busy = false }: { workOrder: WorkOrder; timerNow: string; onDetail: (workOrder: WorkOrder) => void; busy?: boolean }) {
  ...
  return <article className={`requester-account-card status-${workOrder.status}`} role="button" tabIndex={busy ? -1 : 0} aria-disabled={busy} aria-label={`Open details for ${workOrder.number}`} onClick={openCard} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); openCard(); } }}><div className="card-topline"><strong>{workOrder.number}</strong><StatusBadge status={workOrder.status} /></div><RequesterElapsedTimer workOrder={workOrder} timerNow={timerNow} /><span className="requester-department-chip">{workOrder.responsibleDepartment}</span><h3>{workOrder.machineName || workOrder.location}</h3><p>{workOrder.issueDescription}</p><div className="card-meta"><span>{workOrderTypeLabels[workOrder.type]}</span><span>{workOrder.area}</span>{workOrder.responsibleDepartment === "Production" && workOrder.shiftGroup !== "N/A" ? <span>Shift {workOrder.shiftGroup}</span> : null}</div><footer><time>{formatDateTime(workOrder.updatedAt)}</time><span className="requester-card-open-hint"><Eye size={15} />Tap to open</span></footer></article>;
}

function RequesterDetailDialog({ detail, canVerify, timerNow, onClose, onVerify, actionId, note, onNote }: { detail: WorkOrderDetail; canVerify: boolean; timerNow: string; onClose: () => void; onVerify: (workOrder: WorkOrder, status: "closed" | "returned") => void; actionId: string; note: string; onNote: (note: string) => void; }) {
  ...
  return <><div className="requester-detail-backdrop"><section className="requester-detail-dialog" role="dialog" aria-modal="true" aria-labelledby="requester-detail-title"><button className="requester-dialog-close" type="button" onClick={onClose} aria-label="Close details"><X size={18} /></button><div className="requester-detail-title"><p>{detail.number}</p><h2 id="requester-detail-title">{detail.machineName || detail.location}</h2><StatusBadge status={detail.status} /></div><p className="requester-detail-issue">{detail.issueDescription}</p><RequesterElapsedTimer workOrder={detail} timerNow={timerNow} detailed /><dl><div><dt>Responsible department</dt><dd>{detail.responsibleDepartment}</dd></div><div><dt>Area</dt><dd>{detail.area}</dd></div><div><dt>Category</dt><dd>{detail.issueCategoryName || detail.issueCategory?.name || "General"}</dd></div><div><dt>Assigned to</dt><dd>{detail.assignedTo?.name || "Waiting for assignment"}</dd></div><div><dt>Maintenance actual</dt><dd>{formatMinutes(detail.maintenanceActualMinutes)}</dd></div><div><dt>Opened</dt><dd>{formatDateTime(detail.createdAt)}</dd></div><div><dt>Updated</dt><dd>{formatDateTime(detail.updatedAt)}</dd></div></dl>{detail.completionNote ? <div className="requester-completion-note"><strong>Maintenance completion note</strong><p>{detail.completionNote}</p></div> : null}{detail.productionDowntimeReason ? <div className="requester-completion-note"><strong>Production downtime explanation</strong><p>{detail.productionDowntimeReason}</p></div> : null}<div className="requester-detail-photos"><h3>Photos</h3>{detail.attachments.length ? <div>{detail.attachments.map((attachment) => { const label = attachment.kind.replace("_", " "); return <button type="button" key={attachment.id} onClick={() => setPreviewPhoto({ src: mediaUrl(attachment.url), alt: attachment.originalName, label })}><img src={mediaUrl(attachment.url)} alt={attachment.originalName} /><span>{label}</span></button>; })}</div> : <p>No photos uploaded.</p>}</div><div className="requester-detail-timeline"><h3>Timeline</h3>{detail.activities.map((activity) => <article key={activity.id}><span /><div><strong>{activity.message}</strong><time>{formatDateTime(activity.createdAt)}</time></div></article>)}</div>{detail.status === "resolved" && canVerify ? <div className="requester-detail-verification"><label>{needsReason ? "Production downtime explanation" : "Verification note"}<textarea rows={3} value={note} onChange={(event) => onNote(event.target.value)} placeholder={needsReason ? "Required before closing this extended downtime" : "Required if returning to maintenance"} /></label><div className="requester-verification-actions"><button type="button" className="verify" disabled={Boolean(actionId) || (needsReason && !note.trim())} onClick={() => onVerify(detail, "closed")}><CheckCircle2 size={17} />Verify & Close</button><button type="button" className="return" disabled={Boolean(actionId)} onClick={() => onVerify(detail, "returned")}><RefreshCcw size={17} />Return</button></div></div> : null}</section></div>{previewPhoto ? <ImageLightbox {...previewPhoto} onClose={() => setPreviewPhoto(null)} /> : null}</>;
}

function RequesterElapsedTimer({ workOrder, timerNow, detailed = false }: { workOrder: WorkOrder; timerNow: string; detailed?: boolean }) {
  const running = !workOrder.resolvedAt && !["closed", "cancelled"].includes(workOrder.status);
  const end = workOrder.resolvedAt || (running ? timerNow : workOrder.updatedAt);
  const label = running ? "Live production downtime" : workOrder.resolvedAt ? "Production downtime" : "Time before cancellation";
  return <div className={`requester-elapsed-timer ${running ? "is-live" : "is-stopped"} ${detailed ? "is-detailed" : ""}`}><Clock3 size={detailed ? 21 : 17} aria-hidden="true" /><span><small>{label}</small><strong>{formatLiveDuration(workOrder.createdAt, end)}</strong></span>{detailed ? <time>{running ? "Counting until maintenance resolves" : workOrder.resolvedAt ? `Resolved ${formatDateTime(workOrder.resolvedAt)}` : `Cancelled ${formatDateTime(workOrder.updatedAt)}`}</time> : null}</div>;
}
